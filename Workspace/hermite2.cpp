...
void DrawHermite(HDC hdc, const vector<Point> &controlPoints, COLORREF color, int number_of_points)
{
    vector<vector<int>> hermiteMatrix = {
        {2, 1, 1, -2},
        {-3, -2, -1, 3},
        {0, 1, 0, 0},
        {1, 0, 0, 0},
    };
    vector<Point> result = multiply_matrix(hermiteMatrix, controlPoints);
...
LRESULT WndProc(HWND hwnd, UINT m, WPARAM wp, LPARAM lp)
{
    ...
    case WM_RBUTTONDOWN:
    {
        if (counter < 4)
            break;
        hdc = GetDC(hwnd);
        DrawHermite(hdc, points, RGB(255, 0, 0), 1000);
        ReleaseDC(hwnd, hdc);
        points.clear();
        counter = 0;
        break;
